// ViewModel для управления картой и списком локаций.
// Хранит массив локаций, выбранную локацию, текущий регион карты и состояние видимости списка.
// Обеспечивает навигацию по локациям и синхронизацию UI через @Observable.
...
@Observable class LocationsViewModel {
    ...
    /// Шаги:
    /// 1) Загружаем массив локаций из сервиса.
    /// 2) Выбираем первую локацию как текущую.
    /// 3) Обновляем регион карты под эту локацию.
    /// Предположение: массив локаций не пуст.
    init() {
